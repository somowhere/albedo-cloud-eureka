...
@ToString
@NoArgsConstructor
public class TableEntity extends IdEntity<TableEntity> {
    ...
    private List<TableColumnEntity> columnList; // 表列

    @JSONField(serialize = false)
    @ManyToOne(name = "parent_table")
    @TableField(exist = false)
    private TableEntity parent; // 父表对象

    @TableField(exist = false)
    private List<TableEntity> childList; // 子表列表
    ...
    @TableField(exist = false)
    @JSONField(serialize = false)
    private List<TableColumnEntity> pkColumnList; // 当前表主键列表
    @TableField(exist = false)
    private String category; // 当前表的生成分类
    @TableField(exist = false)
    @JSONField(serialize = false)
    private List<TableColumnEntity> columnFormList;
    ...
    public TableEntity(String name, String comments) {
        this.name = name;
