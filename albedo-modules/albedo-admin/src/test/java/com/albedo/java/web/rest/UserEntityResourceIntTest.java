...
public class UserEntityResourceIntTest {
     ...
     * if they test an domain which has a required relationship to the UserEntity domain.
     */
    public UserDataVo createEntity() {
        ...
        user.setRoleIdList(CollUtil.extractToList(roleEntityList, RoleEntity.F_ID));
        return user;
    }
    ...
    public void initTest() {
		...
		anotherUser.setRoleIdList(CollUtil.extractToList(roleEntityList, RoleEntity.F_ID));
    ...
    public void createUser() throws Exception {
        List<UserEntity> databaseSizeBeforeCreate = userService.list();
		...
		UserEntity testUserEntity = userService.findOne(Wrappers.<UserEntity>query().lambda()
		...
		assertThat(testUserEntity.getQqOpenId()).isEqualTo(DEFAULT_QQOPENID);
		assertThat(testUserEntity.getDelFlag()).isEqualTo(UserEntity.FLAG_NORMAL);
    }
    ...
    public void getUserPage() throws Exception {
        ...
        restUserMockMvc.perform(get(DEFAULT_API_URL)
			.param(PageModel.F_DESC, UserEntity.F_SQL_CREATEDDATE)
...
//			.andExpect(jsonPath("$.data.records.[*].status").value(hasItem(DictUtil.getCode("sys_status", userEntity.getStatus()))))
	...
	public void getUserInfo() throws Exception {
		...
		restUserMockMvc.perform(get(DEFAULT_API_URL+"/info/{username}", user.getUsername()))
			...
			.andExpect(jsonPath("$.data.roles").value(equalTo(CollUtil.extractToList(roleEntityList, RoleEntity.F_ID))))
    ...
    public void updateUser() throws Exception {
        ...
        UserEntity updatedUserEntity = userService.findOneById(user.getId());
		...
		managedUserVM.setRoleIdList(CollUtil.extractToList(roleEntityList, RoleEntity.F_ID));
        ...
        UserEntity testUserEntity = userService.findOneById(updatedUserEntity.getId());
		...
		assertThat(testUserEntity.getLockFlag()).isEqualTo(UPDATED_LOCKFLAG);
        assertThat(testUserEntity.getPhone()).isEqualTo(UPDATED_PHONE);
        assertThat(testUserEntity.getQqOpenId()).isEqualTo(UPDATED_QQOPENID);
    }
	...
	public void lockOrUnLockUser() throws Exception {
		...
		// Validate the database is empty
		UserEntity tempUserEntity = userService.findOneById(user.getId());
		assertThat(CommonConstants.STR_YES.equals(tempUserEntity.getLockFlag()));
		// lockOrUnLock the userEntity
		restUserMockMvc.perform(put(DEFAULT_API_URL+"{id}", user.getId())
			.accept(TestUtil.APPLICATION_JSON_UTF8))
			.andExpect(status().isOk());
		...
		// Validate the database is empty
		UserEntity tempUserEntity1 = userService.findOneById(user.getId());
		assertThat(CommonConstants.STR_NO.equals(tempUserEntity1.getLockFlag()));
	}
    @Test
    @Transactional
