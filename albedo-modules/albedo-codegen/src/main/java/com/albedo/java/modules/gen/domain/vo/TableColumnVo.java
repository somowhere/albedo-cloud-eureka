...
public class TableColumnVo extends DataEntityVo<String> implements Comparable {
     ...
     * 是否主键（1：主键）
     */
    private Integer isPk = CommonConstants.NO;
    /**
     * 是否唯一（1：是；0：否）
     */
    private Integer isUnique = CommonConstants.NO;
    /**
     * 是否可为空（1：可为空；0：不为空）
     */
    private Integer isNull = CommonConstants.NO;
    /**
     * 是否为插入字段（1：插入字段）
     */
    private Integer isInsert = CommonConstants.NO;
    /**
     * 是否编辑字段（1：编辑字段）
     */
    private Integer isEdit = CommonConstants.NO;
    /**
     * 是否列表字段（1：列表字段）
     */
    private Integer isList = CommonConstants.NO;
    /**
     * 是否查询字段（1：查询字段）
     */
    private Integer isQuery = CommonConstants.NO;
    /**
    ...
    public boolean getPk() {
        return CommonConstants.YES.equals(getIsPk());
    }

    public boolean getUnique() {
        return CommonConstants.YES.equals(getIsUnique());
    }

    public boolean getNullable() {
        return CommonConstants.YES.equals(getIsNull());
    }
    ...
    public List<String> getAnnotationList() {
        ...
        // 导入JSR303验证依赖包
        if (!"1".equals(getIsNull()) && !CommonConstants.TYPE_STRING.equals(getJavaType())) {
            list.add("javax.validation.constraints.NotNull(message=\"" + getTitle() + "不能为空\")");
        } else if (!"1".equals(getIsNull()) && CommonConstants.TYPE_STRING.equals(getJavaType()) && !"0".equals(getDataLength())) {
